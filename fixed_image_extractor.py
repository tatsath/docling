#!/usr/bin/env python3
"""Fixed image extraction using get_image() method"""
...
# Process PDF
print("🚀 GPU Processing with Fixed Image Extraction...")
...
# Extract and save images using get_image() method
...
if hasattr(doc, 'pictures') and doc.pictures:
    ...
    for i, picture in enumerate(doc.pictures):
        try:
            # Use get_image() method instead of image attribute
            img_data = picture.get_image()
            ...
            if img_data:
                ...
                img_info = {
                    "index": i+1,
                    "path": img_path,
                    "caption": getattr(picture, 'caption_text', '') or getattr(picture, 'captions', ''),
                    "alt_text": getattr(picture, 'alt_text', ''),
                    "size": img_data.size if hasattr(img_data, 'size') else None,
                    "format": img_data.format if hasattr(img_data, 'format') else None
                }
                images_info.append(img_info)
                
                print(f"✅ Saved image {i+1}: {img_path} ({img_data.size if hasattr(img_data, 'size') else 'Unknown size'})")
            else:
                print(f"❌ No image data for picture {i+1}")
...
if images_info:
    ...
    for img in images_info:
        ...
        enhanced_markdown += f"*Size: {img['size']} | Format: {img['format']}*\n\n"
